...
const getTables = catchAsync(async (req: TypedRequest<{}, {}>, res: Response) => {
  const { user } = req
  const currentTables = CurrentTables.tables.filter(u => !u.players.find(opp => opp.user_id === String(user._id))).map(table => ({ ...table, players: table.players.map(user => user.user) }))
